...
BOOL CWca3::Load(char *pFileName)
{
    size_t len =0;
	int nEncodeDataSize;
	struct stat sb;
    ...
    len=fread(m_pEncodedData,sizeof(char),nEncodeDataSize,fd);
	if( (unsigned)len < (unsigned)nEncodeDataSize){
	...
	m_pImageIntArray = new BYTE[m_StreamInfo.iMaxHeight * m_StreamInfo.iMaxWidth];
...
int CWca3::Decode(int nFrame)
{
	...
	for (int i=0; i<m_nFrameNum; i++) 
	{
		...
		m_nFrameDelay = m_FrmInfo.iDuration;
		if( i == nFrame)
		{	// frame found
	...
	m_DecodeParam.eColorSpace						= WCI3_CSP_BGRA;				// 32-bit BGRA packed
